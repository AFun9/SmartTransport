...
nx = 9
ny = 6
file_paths = glob.glob(r".\camera_cal\calibration*.jpg") 
...
def cal_calibrate_params(file_paths):
    ...
    for file_path in file_paths:
        ...
        if gray is None:
            print(f"Failed to convert {file_path} to grayscale.")
            continue
        # 角点检测
    ...
    if gray is None:
        print("No images were successfully read and converted to grayscale.")
        return None, None, None, None, None
...
def img_undistort(img, mtx, dist):
    if img is None:
        print("Error: Input image is None in img_undistort")
        return None
    if mtx is None or dist is None:
        print("Error: Camera matrix or distortion coefficients are None")
        return img.copy()  # 返回原始图像的副本，或者根据需要返回 None
    dis = cv2.undistort(img.copy(), mtx, dist, None, mtx)
    return dis
...
if __name__ == "__main__":
    ...
    if mtx is None or dist is None:
        print("Error: Failed to calibrate camera. Cannot proceed.")
        exit()

    # 测试透视变换
    img = cv2.imread(r'.\test\straight_lines2.jpg')
    if img is None:
...
# 视频处理
clip1 = VideoFileClip("laneDection/project_video.mp4")
